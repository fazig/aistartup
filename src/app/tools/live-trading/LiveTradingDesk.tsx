...
function heatColor(pct: number) {
  ...
  if (t >= 0) {
    const a = 0.28 + t / 10;
    return `rgba(62, 224, 196, ${a})`;
  }
  const a = 0.28 + Math.abs(t) / 10;
  return `rgba(255, 107, 122, ${a})`;
}
  ...
  return (
    <section className="pulse-desk">
        ...
        <div className="pulse-tape-track">
          {tapeAssets.map((asset, i) => {
            ...
            return (
              <span
                key={`${asset.id}-${i}`}
                className={`pulse-chip ${tone(row.changePercent)}`}
              >
                <b>{asset.display}</b>
      ...
      <div className="pulse-inner">
        <Link href="/tools" className="pulse-kicker" style={{ marginBottom: "1rem", display: "inline-flex" }}>
        ...
        <div className="pulse-top">
          <div>
            <div className="pulse-kicker">
              <span
                className={`pulse-dot ${status === "polling" ? "is-poll" : ""} ${status === "offline" ? "is-off" : ""}`}
              />
              {status === "live" ? "Live wire" : status === "polling" ? "Polling feed" : "Feed paused"}
            </div>
            <h1>Pulse Desk</h1>
            <p className="pulse-lede">
              A nocturne trading floor for crypto, stocks, forex, and metals — realtime ticks,
              a living sparkline, and a heat mosaic instead of a generic terminal.
            </p>
          </div>
          <div className="pulse-meta">
            <div className="pulse-pill">
              <Radio size={13} /> {status === "live" ? "Binance socket" : "Public market feeds"}
            </div>
            <div className="pulse-pill">
              <Clock />
            </div>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", gap: "0.75rem", flexWrap: "wrap" }}>
          ...
          <label style={{ position: "relative", display: "inline-flex", alignItems: "center" }}>
            <Search size={14} style={{ position: "absolute", left: 12, color: "#8d929d" }} />
            <input
        ...
        <div className="pulse-grid">
          ...
          <div className="pulse-panel">
            <div className="pulse-hero-head">
              <div>
                ...
                <div className="pulse-hero-sub">
                  {selected.display}
                  {selected.source === "binance" ? " · USDT perpetual tape" : " · exchange quote"}
                  {quote?.marketState ? ` · ${quote.marketState.toLowerCase()}` : ""}
                </div>
              </div>
              <div className="pulse-gauge" aria-label="Market pulse">
                <svg viewBox="0 0 80 80">
                  <circle cx="40" cy="40" r="28" fill="none" stroke="rgba(228,192,120,0.15)" strokeWidth="7" />
                  <circle
                    cx="40"
                    cy="40"
                    r="28"
                    fill="none"
                    stroke={pulse >= 0 ? "#3ee0c4" : "#ff6b7a"}
                    strokeWidth="7"
            ...
            <div
              className={`pulse-last ${quote?.flash === "up" ? "is-flash-up" : ""} ${
                quote?.flash === "down" ? "is-flash-down" : ""
              }`}
              aria-live="polite"
            >
              {quote ? formatPrice(quote.price, selected.category) : "—"}
            </div>
            <div className={`pulse-chg ${tone(quote?.changePercent ?? 0)}`} style={{ fontSize: "1rem" }}>
              {quote
          ...
          <aside className="pulse-panel">
            ...
            {selected.source === "binance" ? (
              <>
                  ...
                  <div>
                    {bids.map((level) => (
                      <div key={`b-${level.price}`} className="pulse-lvl is-up">
                        <i style={{ width: `${(level.qty / maxBid) * 100}%`, background: "#3ee0c4" }} />
                  ...
                  <div>
                    {asks.map((level) => (
                      <div key={`a-${level.price}`} className="pulse-lvl is-down">
                        <i style={{ width: `${(level.qty / maxAsk) * 100}%`, background: "#ff6b7a" }} />
                        <span>{formatPrice(level.price, "crypto")}</span>
                        <span>{formatCompact(level.qty)}</span>
                      </div>
                    ))}
                ...
                <h2 style={{ marginTop: "0.9rem" }}>Prints</h2>
              ...
              <p className="pulse-empty">
                Equity and forex quotes refresh every few seconds. Open a crypto pair for the live
                ladder and trade prints.
              </p>
            )}
          </aside>
        </div>
        ...
        <p className="pulse-note">
          Pulse Desk is a market viewer, not a broker. Crypto ticks stream from Binance public
          websockets; stocks, forex, and metals are polled from public quote feeds and may be
